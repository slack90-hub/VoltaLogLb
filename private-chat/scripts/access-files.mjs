...
export async function refreshAccessFiles() {
  ...
  for(const user of ['nad','maria']) {
    ...
    await writeFile(new URL(`${user}-access.md`,directory),`# ${label}'s private room access\n\nOpen https://mds-room.slack-90.workers.dev/\n\nAccount: ${user}\nPassword: ${password}\n\nConversation recovery key:\n${code}\n\nSign in, then paste the recovery key. Keep a safe offline copy. Share only through a trusted private channel.\n\nOn mdslb.com, tap the footer copyright line five times to open the room. Quick exit: Escape or the Quick exit button.\n`);
  }
}
