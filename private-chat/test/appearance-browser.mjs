...
try{
  ...
  for(const mobile of [false,true]){
    ...
    await page.route('https://mdslb.com/assets/room-artwork.png',route=>route.fulfill({path:'../assets/room-artwork.png',contentType:'image/png'}));await page.goto(base);await login();await page.locator('#draft').fill('A message stays unchanged.');await page.locator('#send').click();await page.getByText('A message stays unchanged.',{exact:true}).first().waitFor();
    assert.ok(await page.locator('#timeline').evaluate(el=>getComputedStyle(el,'::before').backgroundImage.includes('room-artwork.png')));await page.screenshot({path:`test-artifacts/personal-${mobile?'mobile':'desktop'}.png`});assert.equal(await page.getByRole('switch').isChecked(),false);assert.equal(await page.locator('.chat-title h1').innerText(),'Lion & Butterfly');
    ...
    assert.equal(await page.locator('#timeline').evaluate(el=>getComputedStyle(el,'::before').backgroundImage),'none');const chrome=await page.evaluate(()=>{const room=document.querySelector('#room').cloneNode(true);room.querySelector('#messages').remove();room.querySelector('#draft').remove();const copy=document.createElement('div');copy.append(room);document.body.append(copy);room.hidden=false;const text=room.innerText;copy.remove();return text;});
